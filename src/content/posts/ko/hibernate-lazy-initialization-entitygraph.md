...
description: LazyInitializationException이 왜 발생하는지, 그리고 @EntityGraph로 가장 깔끔하게 고치는 방법.
tags:
  - backend
  - spring-boot
  - jpa
...
LazyInitializationException: could not initialize proxy
  [com.example.app.entity.Template#8] - no Session
```
...
`row.getTemplate()`을 루프 안에서 호출하니까 트랜잭션 안에서 프록시가 초기화되겠지, 라고 생각하기 쉽다. 하지만 그렇지 않다.
...
`getDefaultTemplatesForCategory`의 트랜잭션이 *종료된 뒤*에 컨트롤러가 `DefaultTemplatesResponse.of(...)`를 호출한다. 그 안에서:

```java
viewTemplate != null ? viewTemplate.getId() : null,    // 안전 — 프록시에 ID 내장
...
Hibernate 프록시는 진짜 엔티티 클래스의 서브클래스다. PK 필드는 프록시 *생성 시점*에 이미 채워진다. 그래서 세션 없이도 접근 가능하다.

그 외 필드는 처음 접근하는 시점에 DB 조회를 트리거한다. 세션이 없으면 그 조회 자체가 불가능해서 `LazyInitializationException`이 난다. 아주 일관된 동작인데, 처음 만나면 설계를 의심하게 된다.
...
이 어노테이션은 *이 쿼리에서만* `template`을 LEFT JOIN FETCH로 가져오게 한다. 엔티티 자체의 기본 fetch 전략(`LAZY`)은 그대로 유지된다.

다른 옵션도 있긴 하다.

- `@ManyToOne(fetch = FetchType.EAGER)` — 엔티티 수준에서 항상 즉시 로딩. 이 연관관계를 쓰는 *모든* 쿼리에 영향이 가니 과하다.
...
`row.getAssociation()`은 프록시를 초기화하지 않는다. *초기화는 ID 외의 필드에 처음 접근할 때*만 일어난다. 트랜잭션이 닫힌 뒤에 그 접근이 일어나면 예외가 난다.

서비스에서 엔티티 그래프를 반환하고 컨트롤러에서 소비하는 패턴을 쓴다면, 트랜잭션 안에서 필요한 연관관계가 모두 초기화되는지 *항상* 확인해야 한다. `@EntityGraph`는 이 확인을 코드 한 줄로 끝낸다.
